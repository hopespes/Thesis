...
double mean(ActionsAndProbs actions) {
  for (auto i : actions) {
   std::cout << i << std::endl;
  }
  double distance = 0;
...
std::unique_ptr<SearchNode> MCTSBot::MCTSearch(const State& state) {
  ...
  std::unique_ptr<State> p_state = state.Clone();
  ActionsAndProbs legal = evaluator_->Prior(*p_state);

  // This leaves the highest TOP_K elements of legal  
  // legal[0] and legal[TOP_K - 1]
  std::partial_sort(legal.begin(), legal.begin() + TOP_K, legal.end(), 
          [](const auto& a, const auto& b) {
            return a.second > b.second;
          }
          );

	double top_k_density = 0;
	for (int i = 0; i < TOP_K; i++) {
		top_k_density += legal[i].second;
	}

	double lower_density = 0;
	for (int i = TOP_K; i < legal.size(); i++) {
		lower_density += legal[i].second;
	}

  int distance = top_k_density - lower_density;

  int n_new = max_simulations_ * 
		// Cofactor
		(2 / (1 + std::exp(-8 * (distance - 0.5))));
