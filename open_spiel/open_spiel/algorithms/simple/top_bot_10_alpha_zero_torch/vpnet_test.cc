...
int main(int argc, char** argv) {
  ...
  open_spiel::algorithms::torch_haz::TestModelCreation("resnet");

  // Tests below here reuse the graphs created above. Graph creation is slow
  // due to calling a separate python process.

  open_spiel::algorithms::torch_haz::TestModelLearnsSimple("resnet");

  auto train_inputs = open_spiel::algorithms::torch_haz::SolveGame();
  open_spiel::algorithms::torch_haz::TestModelLearnsOptimal("resnet",
                                                           train_inputs);
}
