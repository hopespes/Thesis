...
double RandomRolloutEvaluator::Certainty(const State& state) {
	return 0;
}
...
Action MCTSBot::Step(const State& state) {
  ...
  std::unique_ptr<SearchNode> root = MCTSearch(state, &simulations);
...
std::unique_ptr<SearchNode> MCTSBot::MCTSearch(const State& state, 
  ...
  *simulations += max_simulations_;
	...
	*priors = evaluator_->Prior(state);
	double certainty = evaluator_->Certainty(state);

  for (int i = 0; i < max_simulations_ * (certainty + 1); ++i) {
    visit_path.clear();
    returns.clear();
