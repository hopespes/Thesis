...
#include "open_spiel/algorithms/simple/rl_alpha_zero_torch/model.h"
...
ResOutputBlockImpl::ResOutputBlockImpl(const ResOutputBlockConfig& config)
    : value_conv_(torch::nn::Conv2dOptions(
      ...
      certainty_conv_(torch::nn::Conv2dOptions(
                      /*input_channels=*/config.input_channels,
                      /*output_channels=*/config.value_filters,
                      /*kernel_size=*/config.kernel_size)
                      .stride(1)
      ...
      certainty_batch_norm_(
          torch::nn::BatchNorm2dOptions(
              ...
              .affine(true)
              .track_running_stats(true)),
      certainty_linear1_(torch::nn::LinearOptions(
                         /*in_features=*/config.value_linear_in_features,
                         /*out_features=*/config.value_linear_out_features)
                         .bias(true)),
      certainty_linear2_(torch::nn::LinearOptions(
                         /*in_features=*/config.value_linear_out_features,
                         /*out_features=*/1)
                         .bias(true)),
      certainty_observation_size_(config.value_observation_size) 
{
  ...
  register_module("certainty_conv", certainty_conv_);
  register_module("certainty_batch_norm", certainty_batch_norm_);
  register_module("certainty_linear_1", certainty_linear1_);
  register_module("certainty_linear_2", certainty_linear2_);
...
std::vector<torch::Tensor> ResOutputBlockImpl::forward(torch::Tensor x,
  ...
  torch::Tensor certainty_output = torch::relu(certainty_batch_norm_(certainty_conv_(x)));
  certainty_output = certainty_output.view({-1, certainty_observation_size_});
  certainty_output = torch::relu(certainty_linear1_(certainty_output));
  certainty_output = torch::tanh(certainty_linear2_(certainty_output));

  return {value_output, policy_logits, certainty_output};
}
...
std::vector<torch::Tensor> ModelImpl::forward(torch::Tensor x,
  ...
  return {output[0], torch::softmax(output[1], 1), output[2]};
}
...
std::vector<torch::Tensor> ModelImpl::losses(torch::Tensor inputs,
											 ...
											 torch::Tensor certainty_targets) {
  ...
  torch::Tensor certainty_predictions = output[2];
  ...
  torch::Tensor certainty_loss = mse_loss(certainty_predictions, certainty_targets);
  ...
  return {policy_loss, value_loss, certainty_loss, l2_regularization_loss};
}
