...
void TestModelLearnsSimple(const std::string& nn_model) {
  ...
  for (int i = 0; i < 1000; i++) {
    ...
    std::cout << absl::StrFormat(
        "%d: Losses(total: %.3f, policy: %.3f, value: %.3f, certainty: %.3f, l2: %.3f)\n", i,
        loss.Total(), loss.Policy(), loss.Value(), loss.Certainty(), loss.L2());
  ...
  SPIEL_CHECK_GT(losses.front().Certainty(), losses.back().Certainty());
  SPIEL_CHECK_LT(losses.back().Value(), 0.05);
  SPIEL_CHECK_LT(losses.back().Policy(), 0.05);
  SPIEL_CHECK_LT(losses.back().Certainty(), 0.05);
}
...
void TestModelLearnsOptimal(
  ...
  SPIEL_CHECK_LT(losses.back().Certainty(), 0.1);
}
