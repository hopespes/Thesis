...
VPNetModel::LossInfo VPNetModel::Learn(const std::vector<TrainInputs>& inputs) {
  ...
  std::vector<float> raw_certainty_targets(training_batch_size);
  ...
  for (int batch = 0; batch < training_batch_size; ++batch) {
	...
	raw_value_targets[batch] = inputs[batch].certainty;
  }
  ...
  torch::Tensor torch_certainty_targets =
      torch::from_blob(raw_certainty_targets.data(), {training_batch_size, 1})
          .clone()
          .to(torch_device_);
  ...
  std::vector<torch::Tensor> torch_outputs =
      model_->losses(torch_train_inputs, 
			  torch_train_legal_mask,
			  torch_policy_targets, 
			  torch_value_targets,
			  torch_certainty_targets
			  );

  torch::Tensor total_loss =
      torch_outputs[0] + torch_outputs[1] + torch_outputs[2] + torch_outputs[3];
