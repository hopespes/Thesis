...
#ifndef OPEN_SPIEL_ALGORITHMS_RL_ALPHA_ZERO_TORCH_VPNET_H_
#define OPEN_SPIEL_ALGORITHMS_RL_ALPHA_ZERO_TORCH_VPNET_H_
...
class VPNetModel {
 public:
  ...
  class LossInfo {
   public:
    LossInfo() {}
    LossInfo(double policy, double value, double certainty, double l2)
        : policy_(policy), value_(value), certainty_(certainty), l2_(l2), batches_(1) {}
    ...
    double Value() const { return value_ / batches_; }
		double Certainty() const { return certainty_ / batches_; }
  ...
  struct TrainInputs {
    ...
    NOP_STRUCTURE(TrainInputs, legal_actions, observations, policy, value, certainty);
  };
