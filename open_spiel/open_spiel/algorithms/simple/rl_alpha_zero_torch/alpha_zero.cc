...
#include "open_spiel/algorithms/simple/rl_alpha_zero_torch/alpha_zero.h"
...
#include "open_spiel/algorithms/simple/rl_alpha_zero_torch/device_manager.h"
#include "open_spiel/algorithms/simple/rl_alpha_zero_torch/vpevaluator.h"
#include "open_spiel/algorithms/simple/rl_alpha_zero_torch/vpnet.h"
#include "open_spiel/algorithms/simple/rl_alpha_zero_torch/mcts.h"
...
struct ProbabilityAndFrequency {
	double probability;
	int frequency;
};
		...
		ActionsAndProbs policy) 
{
	...
	std::unordered_map<double, int> frequencies_prd;
	std::unordered_map<double, int> frequencies_pol;

	for (int i = 0; i < n - 1; i++) 
	{
		frequencies_prd[prediction[i].second]++;
		frequencies_pol[policy[i].second]++;

		for (int j = i+1; j < n; j++)
		{
			double x_i = prediction[i].second - prediction[j].second;
			double x_j = policy[i].second - policy[j].second;
			double product = x_i * x_j;
	...
	frequencies_prd[prediction[n - 1].second]++;
	frequencies_pol[policy[n - 1].second]++;

	int nx = 0;
	for (auto const& [key, frequency] : frequencies_prd) 
	{
		nx = nx + frequency * (frequency - 1) / 2;
	}

	int ny = 0;
	for (auto const& [key, frequency] : frequencies_pol)
	{
		ny = ny + frequency * (frequency - 1) / 2;
	}
...
Trajectory PlayGame(Logger* logger, int game_num, const open_spiel::Game& game,
  ...
  Trajectory trajectory;
	...
	unsigned long long start_simulations = simulations->load();
  ...
  while (true) {
    if (state->IsChanceNode()) {
      ...
      open_spiel::Player player = state->CurrentPlayer();
			ActionsAndProbs inferred;
      std::unique_ptr<SearchNode> root = 
          (*bots)[player]->MCTSearch(*state, simulations, &inferred);
      open_spiel::ActionsAndProbs policy;
      ...
      trajectory.states.push_back(Trajectory::State{
          state->ObservationTensor(), 
					...
					std::move(policy), 
					root_value,
					/*certainty=*/krcc(inferred, policy)
			});
  ...
  logger->Print("Game %d: Simulations: %llu; Returns: %s; Actions: %s", game_num,
                *simulations - start_simulations,
...
void evaluator(const open_spiel::Game& game, const AlphaZeroConfig& config,
  ...
  for (int game_num = 1; !stop->StopRequested(); ++game_num) {
    ...
    std::atomic<unsigned long long> simulations = 0;
...
void learner(const open_spiel::Game& game, const AlphaZeroConfig& config,
  ...
  for (int step = start_info.start_step;
    ...
    unsigned long long all_simulations = 0;

    for (int i = 0; i < config.actors; i++)  
    {
        // std::cout << "simulations of actor " << i << " " << simulations[i].load(std::memory_order_relaxed) << std::endl;
        all_simulations += simulations[i].load(std::memory_order_relaxed);
    }
    absl::Time now = absl::Now();
    double seconds = absl::ToDoubleSeconds(now - last);
    std::cout << "The learner is learning" << std::endl;
    std::cout << "Step " << step << std::endl;
    printf("Step: %d\n", step);
    logger.Print("Step: %d", step);
    logger.Print(
        "Collected %5d states from %3d games, simulations: %llu, %.1f states/s; "
        "%.1f states/(s*actor), game length: %.1f, simulations: %llu",
        num_states, 
        num_trajectories, 
		all_simulations,
        num_states / seconds,
    ...
    // logger.Print("Simulations: %lld", simulations)
    ...
    logger.Print("Losses: policy: %.4f, value: %.4f, certainty: %.4f, l2: %.4f, sum: %.4f",
                 losses.Policy(), losses.Value(), losses.Certainty(), losses.L2(), losses.Total());
...
bool AlphaZero(AlphaZeroConfig config, StopToken* stop, bool resuming) {
  ...
  std::atomic<unsigned long long>* simulations = new std::atomic<unsigned long long>[config.actors];
  ...
  for (int i = 0; i < config.actors; ++i) {
    actors.emplace_back(
        [&, i]() { actor(*game, config, i, &trajectory_queue, eval, stop, &simulations[i]); });
  }
  ...
  learner(*game, config, &device_manager, eval, &trajectory_queue,
          &eval_results, stop, start_info, simulations);
