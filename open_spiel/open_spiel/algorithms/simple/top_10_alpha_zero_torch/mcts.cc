...
std::unique_ptr<SearchNode> MCTSBot::MCTSearch(const State& state) {
  ...
  // Calculate entropy
	...
	for (int i = 0; i < TOP_K; i++) {
		H += H - legal[i].second * std::log(legal[i].second);
	}

	double H_norm = H / std::log(TOP_K);

  // Determine update number of simulations
  int n_new = max_simulations_ * 
		// Cofactor
		(2 / (1 + std::exp(-8 * (H_norm - 0.5))));

  //                  max_simulations_ to simulations
...
std::unique_ptr<SearchNode> MCTSBot::MCTSearch(const State& state, 
        ...
        //std::atomic<unsigned long long> *overflow_sim) {
        ) {
  nodes_ = 1;
  gc_limit_ = MIN_GC_LIMIT;
  auto root = std::make_unique<SearchNode>(kInvalidAction,
