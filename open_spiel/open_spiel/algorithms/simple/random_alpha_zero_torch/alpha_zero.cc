...
#include "open_spiel/algorithms/simple/random_alpha_zero_torch/alpha_zero.h"
...
#include "open_spiel/algorithms/simple/random_alpha_zero_torch/device_manager.h"
#include "open_spiel/algorithms/simple/random_alpha_zero_torch/vpevaluator.h"
#include "open_spiel/algorithms/simple/random_alpha_zero_torch/vpnet.h"
#include "open_spiel/algorithms/simple/random_alpha_zero_torch/mcts.h"
  ...
  while (true) {
    if (state->IsChanceNode()) {
      ...
      std::unique_ptr<SearchNode> root = 
          (*bots)[player]->MCTSearch(*state, simulations, dist);
      open_spiel::ActionsAndProbs policy;
...
void actor(const open_spiel::Game& game, const AlphaZeroConfig& config, int num,
  ...
  std::uniform_int_distribution<int> sim_dist(1.0, 300.0);
  std::vector<std::unique_ptr<MCTSBot>> bots;
  bots.reserve(2);
...
void evaluator(const open_spiel::Game& game, const AlphaZeroConfig& config,
  ...
  std::uniform_int_distribution<int> sim_dist(1, 300);
