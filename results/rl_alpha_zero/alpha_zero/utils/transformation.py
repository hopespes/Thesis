...
def rotate_90(
    states: torch.Tensor, pi_probs: torch.Tensor, values: torch.Tensor, certainties: torch.Tensor
) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    states, pi_probs = apply_rotation(states, pi_probs, 90)
    return states, pi_probs, values, certainties
...
def v_flip(
    states: torch.Tensor, pi_probs: torch.Tensor, values: torch.Tensor, certainties
) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
...
def apply_random_transformation(
    ...
    if random.random() > 0.5:
        ...
        states, pi_probs, values, certainties = SUPPORTED_TRANSFORMATIONS[transformation](states, pi_probs, values, certainties)
