...
def parallel_uct_search(
    ...
    warm_up: bool = False,
    deterministic: bool = False,
) -> Tuple[int, np.ndarray, float, float, Node, int, float]:
    """Single-threaded Upper Confidence Bound (UCB) for Trees (UCT) search without any rollout.
    ...
    certainty = 0.0
    if root_node is None:
        prior_prob, value, certainty = eval_func(env.observation(), False)
        root_node = Node(to_play=env.to_play, num_actions=env.action_dim, parent=DummyNode())
        expand(root_node, prior_prob)
        backup(root_node, value)
    ...
    num_simulations = math.floor(num_simulations * np.exp(certainty) + num_parallel)
    ...
    while root_node.N < num_simulations:
        ...
        if leaves:
            ...
            prior_probs, values, certainty = eval_func(np.stack(batched_obs, axis=0), True)
    ...
    tau = safe_kendalltau(search_pi, root_node.child_P)

    return (move, search_pi, root_node.Q, best_child_Q, next_root_node, num_simulations, tau)
