...
from rl_alpha_zero.core.mcts_v2 import Node, parallel_uct_search, uct_search

from rl_alpha_zero.envs.base import BoardGameEnv
from rl_alpha_zero.core.eval_dataset import build_eval_dataset
from rl_alpha_zero.core.rating import EloRating
from rl_alpha_zero.core.replay import UniformReplay, Transition
from rl_alpha_zero.utils.csv_writer import CsvWriter
from rl_alpha_zero.utils.transformation import apply_random_transformation
from rl_alpha_zero.utils.util import Timer, create_logger, get_time_stamp
...
def play_and_record_one_game(
    ...
    while not done:  # For each step
        (move, search_pi, root_Q, best_child_Q, root_node, simulations, tau) = mcts_player(
            env=env,
            root_node=root_node,
            c_puct_base=c_puct_base,
            c_puct_init=c_puct_init,
        ...
        game_simulations = game_simulations + simulations

        episode_states.append(obs)
        episode_search_pis.append(search_pi)
        episode_values.append(0.0)
        episode_certainties.append(tau)
    ...
    game_seq = [
        Transition(state=x, pi_prob=pi, value=v, certainty=c) for x, pi, v, c in zip(episode_states, episode_search_pis, episode_values, episode_certainties)
    ]
        ...
        try:
            ...
            if replay.num_games_added == min_games or (
                ...
                while training_steps < target_t:
                    ...
                    optimizer.zero_grad()
                    pi_loss, v_loss, c_loss = compute_losses(network, device, transitions, argument_data)
                    loss = pi_loss + v_loss + c_loss
                    loss.backward()
                    optimizer.step()
                    lr_scheduler.step()
                    training_steps += 1
                    ...
                    if training_steps % log_interval == 0 or training_steps % ckpt_interval == 0:
                        stats = {
                            ...
                            'certainty_loss': c_loss.detach().item(),
...
def compute_losses(network, device, transitions, argumentation=False) -> Tuple[torch.Tensor, torch.Tensor]:
    ...
    # [B, ]
    target_c = torch.from_numpy(transitions.certainty).to(device=device, dtype=torch.float32, non_blocking=True)

    if argumentation:
        state, target_pi, target_v, target_c = apply_random_transformation(state, target_pi, target_v, target_c)

    pred_pi_logits, pred_v, pred_c = network(state)
    ...
    # State certainty MSE loss
    certainty_loss = F.mse_loss(pred_c.squeeze(), target_c, reduction='mean')

    return policy_loss, value_loss, certainty_loss
...
def eval_on_pro_games(
    ...
    total_v_mse_loss = 0
    total_examples = 0
    for states, target_pi, target_v, target_c in dataloader:
        ...
        target_c = target_c.to(device=device, non_blocking=True)
        # Forward pass to get model predictions
        policy_logits_pred, value_pred, certainty_pred = network(states)

        policy_pred = torch.softmax(policy_logits_pred, dim=-1)
        value_pred = value_pred.squeeze(-1)
        certainty_pred = certainty_pred.squeeze(-1)

        assert certainty_pred.shape == target_c.shape
        ...
        total_v_mse_loss += F.mse_loss(value_pred, target_v, reduction='sum').item()

        total_c_mse_loss += F.mse_loss(certainty_pred, target_c, reduction='sum').item()
    ...
    value_mse_loss = total_v_mse_loss / total_examples

    certainty_mse_loss = total_c_mse_loss / total_examples

    stats = {
        'certainty_mse_error': certainty_mse_loss,
