...
from rl_alpha_zero.envs.go import GoEnv
from rl_alpha_zero.utils import sgf_wrapper
from rl_alpha_zero.utils.util import create_logger
...
def replay_sgf(sgf_file, num_stack, logger, skip_n=0, min_elo=2100, max_games_per_player=200):  # noqa: C901
    ...
    while node.next is not None and 'TW' not in node.next.properties and 'TB' not in node.next.properties:
        ...
        if env.steps > skip_n:
            history.append((obs, _one_hot(next_move, env.action_dim), value, certainty))
...
def build_eval_dataset(games_dir, num_stack, logger=None) -> TensorDataset:
    ...
    target_c = torch.from_numpy(np.stack(target_c, axis=0)).to(dtype=torch.float32)

    eval_dataset = TensorDataset(states, target_pi, target_v, target_c)
