...
from rl_alpha_zero.core.network import AlphaZeroNet as RlNet
from rl_alpha_zero.core.pipeline import create_mcts_player as rl_player
def mcts_rl_player_builder(network, ckpt_file, device):
    network = network.to(device)
    disable_auto_grad(network)
    load_checkpoint_for_net(network, ckpt_file, device)
    network.eval()
...
def play_one_match(
    ...
    white_player = mcts_rl_player_builder(white_network, white_ckpt, device)
