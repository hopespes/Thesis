...
def parallel_uct_search(
    ...
    n_legal = env.legal_actions.sum()
    
    # Find [top_k] highest-rated moves from the predictor
    find_k = min(n_legal, top_k)
    idx = np.argpartition(root_node.child_P, -find_k)
    bot_k_arr = root_node.child_P[idx[:-find_k]]
    top_k_arr = root_node.child_P[idx[-find_k:]]
    print("top", top_k_arr, "             bot", bot_k_arr)

    # Protect from low (zero) values
    top_k_arr = np.maximum(top_k_arr, 1e-12)

    # Calculate entropyt and normalize
    dis = np.sum(top_k_arr) - np.sum(bot_k_arr)

    num_simulations = math.floor(calc_simulations(num_simulations, dis))
